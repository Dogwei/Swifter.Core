...
namespace Swifter.Tools
{
    ...
    public static class MethodHelper
    {
        ...
        private static uint DelegatePointerOffset;
        ...
        {
            if (DelegatePointerOffset == 0)
            {
                unsafe
                {
                    DelegatePointerOffset = TypeHelper.OffsetOf(typeof(Delegate).GetField(DelegatePointerFieldName,
                        BindingFlags.Instance |
                        BindingFlags.Public |
                        BindingFlags.NonPublic)) + (uint)sizeof(IntPtr);
                }
            }

            return Pointer.GetValue<IntPtr>((IntPtr)((long)Pointer.UnBox(@delegate) + DelegatePointerOffset));
        }

        private static void GetParametersTypes(MethodBase methodInfo, out Type[] parameterTypes, out Type resultType)
        {
        ...
        private unsafe static void ImplObjectInterface(TypeBuilder delegateBuilder, MethodInfo invokeMethod, Type[] parameterTypes, Type resultType)
        {
            ...
            for (int i = 0; i < parameterTypes.Length; ++i)
            {
                ...
                iLGen.Emit(OpCodes.Castclass, item);

                if (item.IsValueType)
                {
                    iLGen.Emit(OpCodes.Unbox_Any, item);
                }
            }
            ...
            {
                iLGen.Emit(OpCodes.Box, resultType);
            }
        ...
        private unsafe static void OverrideDynamicInvokeImpl(TypeBuilder delegateBuilder, MethodInfo invokeMethod, Type[] parameterTypes, Type resultType)
        {
            ...
            for (int i = 0; i < parameterTypes.Length; i++)
            {
                ...
                if (item.IsByRef)
                {
                    ...
                    iLGen.LoadArgument(1);
                    iLGen.LoadConstant(i);
                    iLGen.Emit(OpCodes.Ldelema, typeof(object));
        ...
        private unsafe static void ImplInstanceDynamicImpl(TypeBuilder delegateBuilder, MethodInfo invokeMethod, Type[] parameterTypes, Type resultType)
        {
            if (!(parameterTypes.Length != 0 && (parameterTypes[0].IsClass || parameterTypes[0].IsInterface || (parameterTypes[0].IsByRef && parameterTypes[0].GetElementType().IsValueType))))
            {
                return;
            }

            delegateBuilder.AddInterfaceImplementation(typeof(IInstanceDynamicDelegate));

            var interfaceInvoke = typeof(IInstanceDynamicDelegate).GetMethod(nameof(IInstanceDynamicDelegate.Invoke));
            ...
            delegateBuilder.DefineMethodOverride(dynamicInvokeMethodBuilder, interfaceInvoke);
        }
