...
namespace Swifter.Formatters
{
    ...
    public static class TextFormatterHelper
    {
        ...
        public static void DeserializeTo<T>(this ITextFormatter textFormatter, string text, T obj)
        {
            ...
            {
                DeserializeTo(textFormatter, text, nameWriter);
            }
            else if (dataWriter is IDataWriter<int> indexWriter)
            {
                DeserializeTo(textFormatter, text, indexWriter);
            }
            else
            {
        ...
        sealed class DeserializeToAsInvoker : IAsInvoker
        {
            ...
            public void Invoke<Key>()
            {
                if (text != null)
                {
                    DeserializeTo(textFormatter, text, (IDataWriter<Key>)dataWriter);
                }
                else
                {
                    DeserializeTo(textFormatter, textReader, (IDataWriter<Key>)dataWriter);
                }
            }
        }
    }
